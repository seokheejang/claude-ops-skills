...
1. **요구사항 인터뷰** — 도메인, 입력 인자, 외부 도구, 변경 동반 여부
...
| 파일/디렉토리 | 언제 추가 | 예시 |
|--------------|----------|------|
| `REFERENCE.md` | 본문이 100줄 넘고 **분리 토픽이 1~2개** | mmdraw (문법 규칙) |
| `references/` | **3개 이상 독립 토픽**으로 깊은 도메인 지식 | k8s 관련 (networking, storage, gitops...) |
| `scripts/` | 결정론적 작업: 매번 같은 코드 재생성, 검증, 렌더링 | mmdraw render.sh, rpc-agent cosmos_total_tx.py |
| `assets/` | YAML/JSON 템플릿, 샘플 파일, 정적 자원 | Helm values 템플릿, manifest 샘플 |

**원칙**:
- 시작은 SKILL.md만. 구조는 **필요해질 때 점진적으로 확장**
- references/가 있으면 SKILL.md에 "Reference Guide" 표 두어 어떤 상황에 어떤 reference를 로드할지 명시
